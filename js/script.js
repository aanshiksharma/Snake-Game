...
let food = { x: Math.floor(a + (b - a) * Math.random()), y: Math.floor(a + (b - a) * Math.random()) };
let snakeArr = [
    { x: 15, y: 15 }
];
...
function gameOver(snakeArr) {
    for (let index = 1; index < snakeArr.length; index++) {
        ...
        if (snakeArr[index].x == snakeArr[0].x && snakeArr[index].y == snakeArr[0].y) return true;
    }

    // If the snake bumps into one of the four walls
    if (snakeArr[0].x <= 0 || snakeArr[0].y <= 0 || snakeArr[0].x >= 30 || snakeArr[0].y >= 30) return true;
